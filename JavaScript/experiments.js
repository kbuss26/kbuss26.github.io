...
class Deck {
    ...
    returnPile(somePile) {
        this.#cards = somePile.concat(this.#cards);
        console.log(this.#cards);
        return;
    }
...
function evaluateHand() {
    ...
    hand = [];
    for (let i = 2; i < 7; i++) {
        i === 6 ? hand.push(new Card(14, "Spades")) : hand.push(new Card(i, "Spades"));
    } 
    ...
    let thisRank;
    let uniqueFrequency = uniqueCards[0][1];
    let highestCount = uniqueFrequency;
    let flushSuit = hand[0].getSuitRank();

    for (let i = 1; i < uniqueCards.length; i++) {
        uniqueFrequency = uniqueCards[i][1];
        highestCount = Math.max(highestCount, uniqueFrequency);
        lastRank = uniqueCards[i - 1][0];
        thisRank = uniqueCards[i][0];
        if (uniqueFrequency === 2) numPairs++;
        if ((isStraight) && 
            (thisRank != (lastRank + 1)) && 
            (!((lastRank === 5) && (thisRank === 14)))) {
                isStraight = false;
        }
...
function restartGame() {
    ...
    evalText.innerHTML = "Initial draw";
